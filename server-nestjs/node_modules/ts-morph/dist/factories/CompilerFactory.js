...
var CompilerFactory = /** @class */ (function () {
    ...
    function CompilerFactory(context) {
        ...
        this.context.compilerOptions._onModified(function () {
            var e_1, _a;
     ...
     */
    CompilerFactory.prototype.createSourceFile = function (filePath, sourceFileText, options) {
        sourceFileText = sourceFileText instanceof Function ? utils_1.getTextFromStringOrWriter(this.context.createWriter(), sourceFileText) : sourceFileText || "";
    ...
    CompilerFactory.prototype.getNodeFromCompilerNode = function (compilerNode, sourceFile) {
        ...
        return this.nodeCache.getOrCreate(compilerNode, function () { return createNode.call(_this, kindToWrapperMappings_1.kindToWrapperMappings[compilerNode.kind] || compiler_1.Node); });
        function createNode(ctor) {
            var e_3, _a;
            // ensure the parent is created and increment its wrapped child count
            if (compilerNode.parent != null) {
                var parentNode = this.getNodeFromCompilerNode(compilerNode.parent, sourceFile);
                parentNode._wrappedChildCount++;
            }
            var node = new ctor(this.context, compilerNode, sourceFile);
            var parentSyntaxList = node._getParentSyntaxListIfWrapped();
            if (parentSyntaxList != null)
                parentSyntaxList._wrappedChildCount++;
            if (compilerNode.kind === typescript_1.SyntaxKind.SyntaxList) {
                var count = 0;
                try {
                    for (var _b = tslib_1.__values(node._getChildrenInCacheIterator()), _c = _b.next(); !_c.done; _c = _b.next()) {
                        var _ = _c.value;
                        count++;
                    }
                }
    ...
    CompilerFactory.prototype.createSourceFileFromTextInternal = function (filePath, text, options) {
        var hasBom = utils_1.StringUtils.hasBom(text);
        if (hasBom)
            text = utils_1.StringUtils.stripBom(text);
        var sourceFile = this.getSourceFile(this.createCompilerSourceFileFromText(filePath, text), options);
        if (hasBom)
     ...
     */
    CompilerFactory.prototype.getSourceFile = function (compilerSourceFile, options) {
        var _this = this;
        var wasAdded = false;
        var sourceFile = this.nodeCache.getOrCreate(compilerSourceFile, function () {
            ...
            if (!options.markInProject)
                _this.context.inProjectCoordinator.setSourceFileNotInProject(createdSourceFile);
     ...
     * @param dirPath - Directory path.
     */
    CompilerFactory.prototype.getDirectoryFromPath = function (dirPath, options) {
     ...
     * @param dirPath - Directory path.
     */
    CompilerFactory.prototype.createDirectoryOrAddIfExists = function (dirPath, options) {
        var directory = this.directoryCache.createOrAddIfExists(dirPath);
        if (directory != null && options.markInProject)
            directory._markAsInProject();
        return directory;
    };
     ...
     * @param dirPath - Directory path.
     */
    CompilerFactory.prototype.getDirectoryFromCacheOnlyIfInCache = function (dirPath) {
    ...
    CompilerFactory.prototype.removeCompilerNodeFromCache = function (compilerNode) {
        ...
        if (compilerNode.kind === typescript_1.SyntaxKind.SourceFile) {
            ...
            var wrappedSourceFile = this.sourceFileCacheByFilePath.get(sourceFile.fileName);
